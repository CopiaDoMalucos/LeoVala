...
#######                                             ########
############################################################
require_once("backend/functionsedu.php");
dbconn();
function horaseed(){
	return'
...
if ($_POST["takeupload"] == "yes") {
	...
	if (!$descr)
		$message = "Por favor é obrigatorio preencher a lista de músicas";
	...
	/*if (!is_valid_id($langid))
		$message = "Please be sure to select a torrent language";*/
			    ...
			    ////////// album música
        $musicalautor = $_POST['musicalautor'];	
        if (!$musicalautor)	
          $message = "Por favor você tem que colocar o artista & grupo";
		  
		    ////////// album música
        $musicalbumlt = $_POST['musicalbumlt'];	

        if (!$musicalbumlt)
          $message = "Por favor você tem que colocar o Album";
		////////// album música
		
		////////// link da loja música
		
		 $musicalinklojalt = $_POST['musicalinklojalt'];	
		if (!$musicalinklojalt)
		
        $message = "Por favor você tem que colocar o link da loja oficial";
		////////// link da loja música
         $screens1 = $_POST['screens1'];
               if (!$screens1)
		$message = "Por favor você tem que colocar o link da capa da música";
	...
	/////gerador filmes anos
					...
					//////////Extensão músicas
	$musicaqualidadeid = $_POST['musicaeteste'];				
	if (!is_valid_id($musicaqualidadeid))
        ...
        $filmeresolucao = "";
			///////link da loja
							//////////Qaulidade músicas 
	$musicatensaoid = $_POST['musicatteste'];							
	if (!is_valid_id($musicatensaoid ))
...
$ano = date("y", utc_to_tz_time($row['date']));
	...
	$ret = SQL_Query_exec("INSERT INTO torrents (temposeed, filename, owner, name, filmeresolucao, musicalbum, musicalinkloja, musicalautor, filmeresolucalt, screens1, screens2, screens3, screens4, screens5, descr, filmesinopse, imdb, image1, image2, category, added, mes, dia, ano, info_hash, size, numfiles, save_as, announce, external, nfo, torrentlang, filmeano, filmeaudio, filmeextensao, filmequalidade, filme3d, filmecodecvid, filmecodecaud, filmeidiomaorigi, filmeduracaoh, filmeduracaomi, legenda, apliformarq, aplicrack, musicatensao, musicaqualidade, anon, tube, last_action) VALUES (".sqlesc($temposeed).",".sqlesc($fname).", '".$CURUSER['id']."', ".sqlesc($name).", ".sqlesc($filmeresolucao).",".sqlesc($musicalbumlt).",".sqlesc($musicalinklojalt).",".sqlesc($musicalautor).", ".sqlesc($filmeresolucalt).",".sqlesc($screens1).", ".sqlesc($screens2).", ".sqlesc($screens3).", ".sqlesc($screens4).", ".sqlesc($screens5).", ".sqlesc($descr).", ".sqlesc($filmesinopse).",'".$imdb."', '".$inames[0]."', '".$inames[1]."', '".$catid."', '" . get_date_time() . "', '" . $mes . "' , '" . $dia . "' , '" . $ano . "' , '".$infohash."', '".$torrentsize."', '".$filecount."', ".sqlesc($fname).", '".$announce."', '".$external."', '".$nfo."', '".$langid."', '".$filmeanoid."', '".$filmeaudioid."', '".$filmeextensaoid."', '".$filmequalidadeid."', '".$filme3did."','".$filmecodecvidid."', '".$filmecodecaudid."', '".$filmeidiomaorigiid."', '".$filmeduracaohid."', '".$filmeduracaomiid."', '".$legendaid."','".$apliformarqid."', '".$aplicrackid."', '".$musicaqualidadeid."', '".$musicatensaoid."','$anon', '".$tube."','".get_date_time()."')");
...
?>
<form name="upload" enctype="multipart/form-data" action="torrents-musicas.php?cat=<?php echo $_GET["cat"] ;?>" method="post">
...
///// Extensão músicas
$musicatensaoteteste = "<select name=\"musicaeteste\">\n<option value=\"0\">Escolher</option>\n";

$musicatensaoqt = musicasextlist();
foreach ($musicatensaoqt as $row)
	$musicatensaoteteste .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$musicatensaoteteste .= "</select>\n";

print ("<TR><TD width=50%  align=right  class=tab1_col3>Extensão: *</td><td width=50% align=left class=tab1_col3 >".$musicatensaoteteste."</td></tr>");
/////Extensão músicas

///// Qaulidade músicas 
$musicaqualidadeteste = "<select name=\"musicatteste\">\n<option value=\"0\">Escolher</option>\n";

$musicaqualidadeck = musicasqualilist();
foreach ($musicaqualidadeck as $row)
	$musicaqualidadeteste .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$musicaqualidadeteste .= "</select>\n";

print ("<TR><TD width=50%  align=right  class=tab1_col3>Qualidade: *</td><td width=50% align=left class=tab1_col3 >".$musicaqualidadeteste."</td></tr>");
/////Qaulidade músicas 

print ("<TR><TD width=50%  align=right  class=tab1_col3>Artista / Grupo: *</td><td width=50% align=left class=tab1_col3 ><input type=text name=musicalautor size=60 value=" . $_POST['musicalautor'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Album: *</td><td width=50% align=left class=tab1_col3 ><input type=text name=musicalbumlt size=60 value=" . $_POST['musicalbumlt'] . "><BR>\n</td></tr>");


// screenshots //
print ("<TR><TD width=50%  align=right  class=tab1_col3>Capa: *</td><td width=50% align=left class=tab1_col3 ><input type=text name=screens1 size=60 value=" . $_POST['screens1'] . "><BR>\n</td></tr>");
//screenshots //
print ("<TR><TD width=50%  align=right  class=tab1_col3>Link de loja/site oficial: *</td><td width=50% align=left class=tab1_col3 ><input type=text name=musicalinklojalt size=60 value=" . $_POST['musicalinklojalt'] . "><BR>\n</td></tr>");
