...
foreach (array("passkey","info_hash","peer_id","ip","event","uploaded","downloaded","port","left") as $x) {
                $GLOBALS[$x] = $_GET[$x];
}
...
$userfields = "u.id, u.class, u.uploaded, u.downloaded, u.freeleechuser, u.ip, u.passkey, g.can_download"; //user details to get
...
if ($MEMBERSONLY){
    ...
    if ($user["can_download"] == "no")
        err("Você não tem permissão para baixar.");
	$userid = $user["id"]; //etc
	$seedtime_last = $user["seedtime_last"];
}
...
if (!isset($self)){ //IF PEER IS NOT IN PEERS TABLE DO THE WAIT TIME CHECK
	if ($MEMBERSONLY_WAIT && $MEMBERSONLY){
		...
		if ($elapsed < $wait)
			err("Tempo de Espera (" . ($wait - $elapsed) . " horas) - visitar ".$site_config["SITEURL"]." para mais informações");
		}
	}
    ...
    if (($upthis > 0 || $downthis > 0) && is_valid_id($userid)){
	   ...
	   if ($torrent["freeleech"] == 1 || $user["freeleechuser"] == "yes"){
...
SQL_Query_exec("UPDATE `snatched_t` SET `completed` = '1' WHERE `tid` = '$torrentid' AND `uid` = '$userid' AND `utime` = '" . gmtime() . "'");
}	
}//END COMPLETED
...
// NO EVENT? THEN WE MUST BE A NEW PEER OR ARE NOW SEEDING A COMPLETED TORRENT
    ...
    SQL_Query_exec("UPDATE peers SET ip = " . sqlesc($ip) . ", passkey = " . sqlesc($passkey) . ", port = $port, uploaded = $uploaded, downloaded = $downloaded, to_go = $left, last_action = '".get_date_time()."', ultimoacesso = '".gmtime()."', client = " . sqlesc($agent) . ", seeder = '$seeder' WHERE $selfwhere");
    ...
    $ret = SQL_Query_exec("INSERT INTO peers (connectable, torrent, peer_id, ip, passkey, port, uploaded, downloaded, to_go, started, last_action, ultimoacesso, seeder, userid, client) VALUES ('$connectable', $torrentid, " . sqlesc($peer_id) . ", " . sqlesc($ip) . ", " . sqlesc($passkey) . ", $port, $uploaded, $downloaded, $left, '".get_date_time()."', '".get_date_time()."', '".gmtime()."', '$seeder', '$userid', " . sqlesc($agent) . ")");


if ( ($MEMBERSONLY) && ($seeder == 'no') ) {
         SQL_Query_exec("INSERT INTO `snatched_t` (`uid`, `tid`, `stime`, `utime`) VALUES ('$userid', '$torrentid', '".gmtime()."', '".gmtime()."') ON DUPLICATE KEY UPDATE `utime` = '" . gmtime() . "'");
}
    if ($ret){
        if ($seeder == "yes")
            $updateset[] = "seeders = seeders + 1";
