...
if ($_POST["takeupload"] == "yes") {
	...
	//////////gerador aplicativos
					//////////gerador aplicativos
			$apliformarqid = (int) $_POST["apliformarq"];
	if (!is_valid_id($apliformarqid ))
		$message = "Por favor tem que selecionar o campo formato do arquivo";
		//////////gerador aplicativos
			//////////gerador aplicativos
					//////////gerador aplicativos					
	    $aplicrackid = (int) $_POST["aplicrack"];
	if (!is_valid_id($aplicrackid ))
		$message = "Por favor tem que selecionar o campo crack";
		//////////gerador aplicativos
			//////
		$apliversao = $_POST['apliversao'];	
    if (!$apliversao)
        $message = "Por favor tem que colocar a versão do aplicativo";
///screenshot
...
$ano = date("y", utc_to_tz_time($row['date']));
	...
	$ret = SQL_Query_exec("INSERT INTO torrents (temposeed, filename, owner, name, apliversao, filmeresolucalt, screens1, screens2, screens3, screens4, screens5, descr, filmesinopse, imdb, image1, image2, category, added, mes, dia, ano, info_hash, size, numfiles, save_as, announce, external, nfo, torrentlang, filmeano, filmeaudio, filmeextensao, filmequalidade, filme3d, filmecodecvid, filmecodecaud, filmeidiomaorigi, filmeduracaoh, filmeduracaomi, legenda, apliformarq, aplicrack, anon, tube, last_action) VALUES (".sqlesc($temposeed).",".sqlesc($fname).", '".$CURUSER['id']."', ".sqlesc($name).", ".sqlesc($apliversao).", ".sqlesc($filmeresolucalt).", ".sqlesc($screens1).", ".sqlesc($screens2).", ".sqlesc($screens3).", ".sqlesc($screens4).", ".sqlesc($screens5).", ".sqlesc($descr).", ".sqlesc($filmesinopse).",'".$imdb."', '".$inames[0]."', '".$inames[1]."', '".$catid."', '" . get_date_time() . "', '" . $mes . "' , '" . $dia . "' , '" . $ano . "' , '".$infohash."', '".$torrentsize."', '".$filecount."', ".sqlesc($fname).", '".$announce."', '".$external."', '".$nfo."', '".$langid."', '".$filmeanoid."', '".$filmeaudioid."', '".$filmeextensaoid."', '".$filmequalidadeid."', '".$filme3did."','".$filmecodecvidid."', '".$filmecodecaudid."', '".$filmeidiomaorigiid."', '".$filmeduracaohid."', '".$filmeduracaomiid."', '".$legendaid."','".$apliformarqid."', '".$aplicrackid."','$anon', '".$tube."','".get_date_time()."')");
...
?>
<form name="upload" enctype="multipart/form-data" action="torrents-aplicativos.php?cat=<?php echo $_GET["cat"] ;?>" method="post">
...
///// aplicativos
$apliformarqte = "<select name=\"apliformarq\">\n<option value=\"0\">Escolher</option>\n";

$apliformarqt = aplicatilist();
foreach ($apliformarqt as $row)
	$apliformarqte .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$apliformarqte .= "</select>\n";

print ("<TR><TD width=50%  align=right  class=tab1_col3>Formato do Arquivo: *</td><td width=50%  align=left class=tab1_col3>".$apliformarqte."</td></tr>");
/////Aplicativos

///// aplicativos crack
$aplicaticrackte = "<select name=\"aplicrack\">\n<option value=\"0\">Escolher</option>\n";

$aplicaticrack = aplicaticracklist();
foreach ($aplicaticrack as $row)
	$aplicaticrackte .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$aplicaticrackte .= "</select>\n";

print ("<TR><TD width=50%  align=right  class=tab1_col3>Crack?: *</td><td width=50%  align=left class=tab1_col3>".$aplicaticrackte."</td></tr>");
/////Aplicativos crack



print ("<TR><TD width=50%  align=right  class=tab1_col3>Versão: *</td><td width=50%  align=left class=tab1_col3><input type=text name=apliversao size=10 value=" . $_POST['apliversao'] . "><BR>\n</td></tr>");
// screenshots //
...
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 1: *</td><td width=50%  align=left class=tab1_col3><input type=text name=screens2 size=60 value=" . $_POST['screens2'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 2:  </td><td width=50%  align=left class=tab1_col3><input type=text name=screens3 size=60 value=" . $_POST['screens3'] . "><BR>\n</td></tr>");
