...
///gerador nome fim    
	...
	if (!$filmesinopse)
		$message = "Por favor é obrigatorio preencher a informações adicionais";
...
////// genero jogos
			...
			if (!is_valid_id($generoid))
				$message = "Por favor tem que selecionar o campo genero";
	////// ano de lançamento jogos
			
	///// Idioma jogos
	$filmeidiomaorigiid = (int) $_POST["filmeidiomajogos"];
...
$ano = date("y", utc_to_tz_time($row['date']));
	...
	$ret = SQL_Query_exec("INSERT INTO torrents (temposeed, filename, owner, name, filmeresolucao, filmeresolucalt, screens1, screens2, screens3, screens4, screens5, descr, filmesinopse, imdb, image1, image2, category, added, mes, dia, ano, info_hash, size, numfiles, save_as, announce, external, nfo, torrentlang, filmeano, jogosgenero, jogosformato, filmeidiomaorigi, jogosmultiplay, filmeaudio, filmeextensao, filmequalidade, filme3d, filmecodecvid, filmecodecaud,  filmeduracaoh, filmeduracaomi, legenda, anon, tube, last_action) VALUES (".sqlesc($temposeed).",".sqlesc($fname).", '".$CURUSER['id']."', ".sqlesc($name).", ".sqlesc($filmeresolucao).", ".sqlesc($filmeresolucalt).", ".sqlesc($screens1).", ".sqlesc($screens2).", ".sqlesc($screens3).", ".sqlesc($screens4).", ".sqlesc($screens5).", ".sqlesc($descr).", ".sqlesc($filmesinopse).",'".$imdb."', '".$inames[0]."', '".$inames[1]."', '".$catid."', '" . get_date_time() . "', '" . $mes . "' , '" . $dia . "' , '" . $ano . "' , '".$infohash."', '".$torrentsize."', '".$filecount."', ".sqlesc($fname).", '".$announce."', '".$external."', '".$nfo."', '".$langid."', '".$filmeanoid."', '".$generoid."', '".$formatoid."', '".$filmeidiomaorigiid."', '".$jogosmultiplayid."', '".$filmeaudioid."', '".$filmeextensaoid."', '".$filmequalidadeid."', '".$filme3did."','".$filmecodecvidid."', '".$filmecodecaudid."', '".$filmeduracaohid."', '".$filmeduracaomiid."', '".$legendaid."', '$anon', '".$tube."','".get_date_time()."')");
...
?>
<form name="upload" enctype="multipart/form-data" action="torrents-jogos.php?cat=<?php echo $_GET["cat"] ;?>" method="post">
...
print ("</td></tr>");
print ("<tr><td width=50%  align=right  class=tab1_col3>" . T_("TORRENT_FILE") . ": </td><td align=left class=tab1_col3> <input type='file' name='torrent' size='50' value='" . $_FILES['torrent']['name'] . "' />\n</td></tr>");
print ("<tr><td width=50%  align=right  class=tab1_col3>Torrent nome: </td><td align=left class=tab1_col3><input type='text' name='name' size='60' value='" . $_POST['name'] . "' /><br />Este será retirado. torrent se estiver vazio \n</td></tr>");

print ("<TR><TD width=50%  align=right  class=tab1_col3>Horário de Seed:</td><td align=left class=tab1_col3>".horaseed()."</td></tr>");
///////genero jogos
$genero = "<select name=\"genero\">\n<option value=\"0\">Escolher</option>\n";

$generos = jogosgenerolist();
foreach ($generos as $row)
	$genero .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$genero .= "</select>\n";
print ("<TR><TD width=50%  align=right  class=tab1_col3>Genero:* </td><td align=left class=tab1_col3>".$genero."</td></tr>");
////genero jogos
...
print ("<TR><TD width=50%  align=right  class=tab1_col3>Ano de lançamento: *</td><td align=left class=tab1_col3>".$anoid."</td></tr>");
/////Filme Ano de lançamento



////Idioma jogos:
$filmeidiomajogosi = "<select name=\"filmeidiomajogos\">\n<option value=\"0\">Escolher</option>\n";

$filmeidiomajogosl = filmeidiorilist();
foreach ($filmeidiomajogosl as $row)
	$filmeidiomajogosi .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name"]) . "</option>\n";

$filmeidiomajogosi .= "</select>\n";
print ("<TR><TD width=50%  align=right  class=tab1_col3>Idioma:* </td><td align=left class=tab1_col3>".$filmeidiomajogosi."</td></tr>");
////Idiomas jogos:



// screenshots //
print ("<TR><TD width=50%  align=right  class=tab1_col3>Capa: *</td><td align=left class=tab1_col3><input type=text name=screens1 size=60 value=" . $_POST['screens1'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 1: *</td><td align=left class=tab1_col3><input type=text name=screens2 size=60 value=" . $_POST['screens2'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 2: *</td><td align=left class=tab1_col3><input type=text name=screens3 size=60 value=" . $_POST['screens3'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 3: </td><td align=left class=tab1_col3><input type=text name=screens4 size=60 value=" . $_POST['screens4'] . "><BR>\n</td></tr>");
//screenshots //
