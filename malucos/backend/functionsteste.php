...
function userlogin() {
    ...
    if (!$row || sha1($row["id"].$row["secret"].$row["password"].getip().$row["secret"]) != $_COOKIE["pass"]) {  
		logoutcookie();
		return;
	}
...
function deleteaccount($userid) 
{
	...
	mysql_query("DELETE FROM snatched WHERE userid = $userid");
