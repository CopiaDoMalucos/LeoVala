...
function rightblocks() {
    global $site_config, $CURUSER, $THEME, $LANGUAGE, $TTCache, $blockfilename;  //Define globals
			 ...
			 $idvertorrr = (int) $_GET['id'];
	$row = @mysql_fetch_assoc(@SQL_Query_exec("SELECT image1,image2 FROM torrents WHERE id=$idvertorrr"));
if (!$row){
$blockstorrent = 'blocks';
    }else{
	$blockstorrent = 'blockstorrent';
	
	}
	if (($blocks=$TTCache->get("blocks_right", 0)) === false) {
        $res = SQL_Query_exec("SELECT * FROM $blockstorrent WHERE position='right' AND enabled=1 ORDER BY sort");
        $blocks = array();
        while ($result = mysql_fetch_assoc($res)) {
                $blocks[] = $result["name"];
        }
...
$s = preg_replace("/\[mail\]((\s|.)+?)\[\/mail\]/i","<a href=\"mailto:\\1\" target=\"_blank\">\\1</a>", $s);
  ...
  {
    $s = preg_replace("/\[capa\](http:\/\/[^\s'\"<>]+(\.(jpg|gif|png|jpeg)))\[\/capa\]/i", "<a href='\\1' rel='colorbox-{$pn}'><img style='max-width:400px;' src=\"\\1\" </a><br><small>[ Ver em tamanho real ]</small></a>", $s);
    $cboxelement[ $pn ] = "colorbox-{$pn}";
  }
    ...
    $s = preg_replace("/\[DF\]((\s|.)+?)\[\/DF\]/", "<marquee>\\1</marquee>", $s);
     ...
     $s = preg_replace("/\[video=[^\s'\"<>]*youtube.com.*v=([^\s'\"<>]+)\]/ims", "<object width=\"700\" height=\"440\"><param name=\"movie\" value=\"http://www.youtube.com/v/\\1\"></param><embed src=\"http://www.youtube.com/v/\\1\" type=\"application/x-shockwave-flash\" width=\"650\" height=\"440\"></embed></object>", $s);

     $s = preg_replace("/\[VIDEO=[^\s'\"<>]*youtube.com.*v=([^\s'\"<>]+)\]/ims", "<object width=\"700\" height=\"440\"><param name=\"movie\" value=\"http://www.youtube.com/v/\\1\"></param><embed src=\"http://www.youtube.com/v/\\1\" type=\"application/x-shockwave-flash\" width=\"650\" height=\"440\"></embed></object>", $s);

     // Google Vids [video]http://www.video.google.com[/video]
     $s = preg_replace("/\[video\][^\s'\"<>]*video.google.com.*docid=(-?[0-9]+).*\[\/video\]/ims", "<embed style=\"width:700px; height:440px;\" id=\"VideoPlayback\" align=\"middle\" type=\"application/x-shockwave-flash\" src=\"http://video.google.com/googleplayer.swf?docId=\\1\" allowScriptAccess=\"sameDomain\" quality=\"best\" bgcolor=\"#ffffff\" scale=\"noScale\" wmode=\"window\" salign=\"TL\" FlashVars=\"playerMode=embedded\"> </embed>", $s);

     $s = preg_replace("/\[VIDEO\][^\s'\"<>]*video.google.com.*docid=(-?[0-9]+).*\[\/VIDEO\]/ims", "<embed style=\"width:700px; height:440px;\" id=\"VideoPlayback\" align=\"middle\" type=\"application/x-shockwave-flash\" src=\"http://video.google.com/googleplayer.swf?docId=\\1\" allowScriptAccess=\"sameDomain\" quality=\"best\" bgcolor=\"#ffffff\" scale=\"noScale\" wmode=\"window\" salign=\"TL\" FlashVars=\"playerMode=embedded\"> </embed>", $s);

     //[video]YouTube Vids url[/video]
     $s = preg_replace("/\[video\][^\s'\"<>]*youtube.com.*v=([^\s'\"<>]+)\[\/video\]/ims", "<object width=\"700\" height=\"440\"><param name=\"movie\" value=\"http://www.youtube.com/v/\\1\"></param><embed src=\"http://www.youtube.com/v/\\1\" type=\"application/x-shockwave-flash\" width=\"650\" height=\"440\"></embed></object>", $s);
...
'<iframe width="100%" height="250" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.co.uk/maps?f=d&source=s_d&saddr=\1&output=embed"></iframe>', $s);
     ...
     $s = preg_replace("/\[video2\][^\s'\"<>]*youtube.com.*v=([^\s'\"<>]+)\[\/video2\]/ims", "<object width=\"700\" height=\"440\"><param name=\"movie\" value=\"http://www.youtube.com/v/\\1\"></param><embed src=\"http://www.youtube.com/v/\\1\" type=\"application/x-shockwave-flash\" width=\"650\" height=\"440\"></embed></object>", $s);
