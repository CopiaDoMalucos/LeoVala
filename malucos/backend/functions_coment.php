...
return sprintf( '%d meses, %d semanas, %d dias, %d horas, %d minutos, %d secs',$weeks1, $weeks, $days, $hours, $mins, $secs);
}
...
function get_elapsed_time1($ts){
  ...
  $t = "";
  if ($weeks1 > 0)
    return "$weeks1" . ($weeks1 > 1 ? "123456789123456789" : "");
	  if ($weeks > 0)
    return "$weeks semana" . ($weeks > 1 ? "s" : "");
  if ($days > 0)
    return "$days dia" . ($days > 1 ? "s" : "");
...
$placa="";
    ...
    $text = format_comment($row["text"]);
        if ($row["editedby"])
            $text .= "<p><font size=1 class=small>Editado por <a href=userdetails.php?id=$row[editedby]><b>$row[username]</b></a> $row[editedat] GMT</font></p>\n";
        $edit = null; ///[<a href=pkitcomment.php?action=edit&amp;cid=$row[id]>Edit</a>]" : "") 
        if ($type == "torrent" && $CURUSER["edit_torrents"] == "yes" || $type == "news" && $CURUSER["edit_news"] == "yes" || $CURUSER['id'] == $row['user'] || $CURUSER["class"] == "85" || $CURUSER["class"] == "95") 
            $edit = '[<a href=pkitcomment.php?action=edit&amp;cid='.$row["id"].'>Editar</a>]&nbsp;';
       
        $delete = null;
        if ( $CURUSER["class"] == "85" || $CURUSER["class"] == "95"){
            $delete = '[<a href=pkitcomment.php?action=delete&amp;cid='.$row["id"].'>Apagar</a>]&nbsp;';
        }
        print('<table style="border-collapse: collapse;" class="f-border" align="center" cellpadding="3" cellspacing="0" width="100%">');
        print('<tr class="p-title-coment">');
        print('<td align="center" width="17%">'.$postername.'</td>');
        print('<td align="right" width="83%">' . $edit . $delete . ' Postado: '.date("d-m-Y \\a\\s H:i:s", utc_to_tz_time($row["added"])).'<a id="comment'.$row["id"].'"></a></td>');
        print('</tr>');
        print('<tr valign="top">');
        
   print('<td class="f-border comment-details" align="left" width="17%"><center><b><font class=tipo_forum></font></b><br /><i><font class=tipo_forum>'.$title.'</font></i><br /><img width="120" height="150" src="'.$avatar.'" alt="" /><br /><font class=tipo_forum>'.$grupos_exe.'</font><center><br>'.$placa.'<br><center><font class=tipo_forum>_____________________</font></center><br /><br><font class=tipo_forum>Ratio: </font> <B><font class=tipo_forum> '.$ratio.' </font></B><center><font color=#151515>[</font><font color=#FF0000><B> '.$useruploaded.' </font><font color=#151515>|</font><font color=#00CC00> '.$userdownloaded.' </B></font><font color=#151515>]</font></center><center><font class=tipo_forum>_____________________</font></center></center><center><br><font class=tipo_forum>Lançamentos:  </font><B><font class=tipo_forum> ' . number_format($numtorrents) . ' </font></B></center><center><font class=tipo_forum>Comentários:  <B> ' . number_format($numcomments) . ' </font></B><br></center><br><center><B> '.$online.' </B><br></center><br /><a href="account-details.php?id='.$row["user"].'"><img src="themes/'.$THEME.'/forums/icon_profile.gif" border="" alt="" /></a> <a href="mailbox.php?compose&amp;id='.$row["user"].'"><img src="themes/'.$THEME.'/forums/icon_pm.gif" border="0" alt="" /></a></center></td>');
