...
function deletetorrent($id) {
	...
	SQL_Query_exec("DELETE FROM snatched WHERE torrentid = $id");
...
function deleteaccount($userid) 
{
	...
	SQL_Query_exec("DELETE FROM snatched WHERE userid = $userid");
...
function genrelist17() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  id='120' ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}

function pesquisa_avan() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  parent_cat='Filmes' ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
...
$s = preg_replace("/\[mail\]((\s|.)+?)\[\/mail\]/i","<a href=\"mailto:\\1\" target=\"_blank\">\\1</a>", $s);
  ...
  {
    $s = preg_replace("/\[screem\](http:\/\/[^\s'\"<>]+(\.(jpg|gif|png|jpeg)))\[\/screem\]/i", "<a href='\\1' rel='colorbox-{$pn}'><img style='max-width:400px;' src=\"\\1\" </a><br><small>[ Ver em tamanho real ]</small></a>", $s);
...
//fin tri
	...
	echo '<table d="tabela1" align="center" cellpadding="0" cellspacing="1"><thead><tr>';

	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=category&order=".($sort=="category" && $order=="desc"?"asc":"desc")."\"><font color='white'>".T_("TYPE")."</font></a>".($sort=="category"?$fleche:"")."</td>";
                        break;
                        case 'name':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=name&order=".($sort=="name" && $order=="desc"?"asc":"desc")."\"><font color='white'>".T_("NAME")."</font></a>".($sort=="name"?$fleche:"")."</td>";
                        break;
                        case 'dl':
                                echo "<td class=ttable_head>Down</td>";
                        break;
                        case 'uploader':
                                echo "<td class=ttable_head>".T_("UPLOADER")."</td>";
                        break;
                        case 'comments':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=comments&order=".($sort=="comments" && $order=="desc"?"asc":"desc")."\"><font color='white'>Com</font></a>".($sort=="comments"?$fleche:"")."</td>";
                        break;
                        case 'nfo':
                                echo "<td class=ttable_head>NFO</td>";
                        break;
                        case 'size':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=size&order=".($sort=="size" && $order=="desc"?"asc":"desc")."\"><font color='white'> ".T_("SIZE")."</font></a>".($sort=="size"?$fleche:"")."</td>";
                        break;
                        case 'completed':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=completed&order=".($sort=="completed" && $order=="desc"?"asc":"desc")."\"><font color='white'>C</font></a>".($sort=="completed"?$fleche:"")."</td>";
                        break;
                        case 'seeders':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=seeders&order=".($sort=="seeders" && $order=="desc"?"asc":"desc")."\"><font color='white'>S</font></a>".($sort=="seeders"?$fleche:"")."</td>";
                        break;
                        case 'leechers':
                                echo "<td class=ttable_head><a href=\"$scriptname?$addparam"."sort=leechers&order=".($sort=="leechers" && $order=="desc"?"asc":"desc")."\"><font color='white'>L</font></a>".($sort=="leechers"?$fleche:"")."</td>";
                        break;
					
		}
	}
	if ($wait && !in_array("wait", $cols))
	echo "<td class=ttable_head>".T_("WAIT")."</td>";
	...
	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
				print("<td class='ttable_col2' width='50' align='center'>");
		...
		$char1 = 100; //cut name length 
			...
			if ($row["freeleechuser"] == 'yes'){
					...
					if ($row["freeleech"] == 1)
					$dispname .= " <b>[<font color='#8E388E'>FREE</font>]</b>";
               ...
               if ($row["filmeresolucao"] > 1200 ||  $row["filmeresolucalt"] > 720 )
 {
                ...
                $dispname .= "<b>[<font color='#8B0000 '>HD</font>]</b>";
        }		
								///qualidade 3d
    ...
    $dispname .= "<b>$grupo</b>";
			...
			break;
			case 'dl':
				print("<td class='ttable_col2' align='center'><a href=\"download.php?id=$id\"><img src='" . $site_config['SITEURL'] . "/images/icon_download.gif' border='0' alt=\"Download .torrent\" /></a></td>");
			break;
			case 'uploader':
				echo "<td class='ttable_col2' align='center'>";
			...
			break;
			case 'comments':
				print("<td class='ttable_col2' align='center'><font size='1' face='verdana'><a href='comments.php?type=torrent&amp;id=$id'>" . number_format($row["comments"]) . "</a></font></td>\n");
			break;
			case 'nfo':
				if ($row["nfo"] == "yes")
					print("<td class='ttable_col2' align='center'><a href='nfo-view.php?id=$row[id]'><img src='" . $site_config['SITEURL'] . "/images/icon_nfo.gif' border='0' alt='View NFO' /></a></td>");
				else
					print("<td class='ttable_col2' align='center'>-</td>");
			break;
			case 'size':
				print("<td class='ttable_col2' align='center'>".mksize($row["size"])."</td>\n");
			break;
			case 'completed':    
				print("<td class='ttable_col2' align='center'><font color='orange'><b>".number_format($row["times_completed"])."</b></font></td>");
			break;
			case 'seeders':
				print("<td class='ttable_col2' align='center'><font color='green'><b>".number_format($row["seeders"])."</b></font></td>\n");
			break;
			case 'leechers':
				print("<td class='ttable_col2' align='center'><font color='#ff0000'><b>" . number_format($row["leechers"]) . "</b></font></td>\n");
			break;
			case 'health':
				print("<td class='ttable_col2' align='center'><img src='".$site_config["SITEURL"]."/images/health/health_".health($row["leechers"], $row["seeders"]).".gif' alt='' /></td>\n");
			break;
			case 'external':
				if ($site_config["ALLOWEXTERNAL"]){
					if ($row["external"]=='yes')
						print("<td class='ttable_col2' align='center'>".T_("E")."</td>\n");
					else
						print("<td class='ttable_col2' align='center'>".T_("L")."</td>\n");
				}
			break;
			case 'added':
				print("<td class='ttable_col2' align='center'>".date("d-m-Y H:i:s", utc_to_tz_time($row['added']))."</td>");
			break;
		...
		//Expanding area
		if (count($expandrows)) {
			print("<tr class='t-row'><td class='ttable_col2' colspan='$colspan'><div id=\"kt".$row['id']."\" style=\"margin-left: 2px; display: none;\">");
...
function pager($rpp, $count, $href, $opts = array()) {
    ...
    $mp = $pages - 1;
    $as = "<b onLoad='document.shoutboxform.message.focus()'>&lt;&lt;&nbsp;".T_("PREVIOUS")."</b>";
    if ($page >= 1) {
        ...
        $pager .= $as;
    $pager .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
    $as = "<b onLoad='document.focus()'>".T_("NEXT")."&nbsp;&gt;&gt;</b>";
    if ($page < $mp && $mp >= 0) {
        $pager .= "<a href=\"{$href}page=" . ($page + 1) . "\">";
        $pager .= $as;
        $pager .= "</a>";
...
$resgrupos = mysql_query("SELECT name,image FROM teams WHERE id = ".$arr123853["team"]." LIMIT 1");
...
$grupos_exe ="<font color=#191970><B><center>Membro da Equipe</center><center>".$arrgrupos["name"]."</center><B></font>";
}							
elseif ( $arr1grupos["status"] == 'submoderadores'){ 
$grupos_exe ="<font color=#56690B><B><center>Sub-moderador da Equipe</center><center>".$arrgrupos["name"]."</center><B></font>";
}	
elseif ( $arr1grupos["status"] == 'moderadores'){ 
$grupos_exe ="<font color=#228B22><B><center>Moderador da Equipe</center><center>".$arrgrupos["name"]."</center><B></font>";
} 
else{
$grupos_exe="";
}
   ...
   print('<td class="f-border comment-details" align="left" width="17%"><center><b><font class=tipo_forum>'.$postername.'</font></b><br /><i><font class=tipo_forum>'.$title.'</font></i><br /><img width="120" height="150" src="'.$avatar.'" alt="" /><br /><font class=tipo_forum>'.$grupos_exe.'</font><center><br>'.$placa.'<br><center><font class=tipo_forum>_____________________</font></center><br /><br><font class=tipo_forum>Ratio: </font> <B><font class=tipo_forum> '.$userratio.' </font></B><center><font color=#151515>[</font><font color=#FF0000><B> '.$useruploaded.' </font><font color=#151515>|</font><font color=#00CC00> '.$userdownloaded.' </B></font><font color=#151515>]</font></center><center><font class=tipo_forum>_____________________</font></center></center><center><br><font class=tipo_forum>Lançamentos:  </font><B><font class=tipo_forum> ' . number_format($numtorrents) . ' </font></B></center><center><font class=tipo_forum>Comentários:  <B> ' . number_format($numcomments) . ' </font></B><br></center><br><center><B> '.$online.' </B><br></center><br /><a href="account-details.php?id='.$row["user"].'"><img src="themes/'.$THEME.'/forums/icon_profile.gif" border="" alt="" /></a> <a href="mailbox.php?compose&amp;id='.$row["user"].'"><img src="themes/'.$THEME.'/forums/icon_pm.gif" border="0" alt="" /></a></center></td>');
