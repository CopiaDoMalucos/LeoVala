...
if (function_exists("date_default_timezone_set"))
	date_default_timezone_set("America/Sao_Paulo"); // Do NOT change this. All times are converted to user's chosen timezone.
...
function genrelist1() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  parent_cat='Filmes' ORDER BY parent_cat ASC ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}
...
function utc_to_tz_time_lei ($timestamp=0) {
	GLOBAL $CURUSER, $tzs;
	...
	if (method_exists("DateTime", "setTimezone")) {
		if (!$timestamp)
			$timestamp = get_date_time();
		$date = new DateTime($timestamp, new DateTimeZone("UTC"));
		$date->setTimezone(new DateTimeZone($CURUSER ? $tzs[-180][1] : "Europe/London"));
		return sql_timestamp_to_unix_timestamp($date->format('Y-m-d H:i:s'));
	}
