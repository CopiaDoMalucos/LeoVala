...
$ano = date("y", utc_to_tz_time($row['date']));
	...
	$ret = SQL_Query_exec("INSERT INTO torrents (temposeed, filename, owner, name, screens1, screens2, screens3, screens4, descr, imdb, image1, image2, category, added, mes, dia, ano, info_hash, size, numfiles, save_as, announce, external, nfo, torrentlang, anon, tube, last_action) VALUES (".sqlesc($temposeed).",".sqlesc($fname).", '".$CURUSER['id']."', ".sqlesc($name).",".sqlesc($screens1).", ".sqlesc($screens2).", ".sqlesc($screens3).", ".sqlesc($screens4).", ".sqlesc($descr).",'".$imdb."', '".$inames[0]."', '".$inames[1]."', '".$catid."', '" . get_date_time() . "', '" . $mes . "' , '" . $dia . "' , '" . $ano . "' , '".$infohash."', '".$torrentsize."', '".$filecount."', ".sqlesc($fname).", '".$announce."', '".$external."', '".$nfo."', '".$langid."','$anon',  '".$tube."','".get_date_time()."')");
...
<form name="upload" enctype="multipart/form-data" action="torrents-upload.php?cat=<?php echo $_GET["cat"] ;?>" method="post">
