...
if ( $CURUSER['id'] == $user['id'] || $CURUSER["level"]=="Administrador"  || $CURUSER["level"]=="Moderador" || $CURUSER["level"]=="S.Moderador" || $CURUSER["level"]=="Sysop" ) {
...
begin_framec("Baixando no momento");


	$res = SQL_Query_exec("SELECT DISTINCT  torrent,uploaded,downloaded FROM peers WHERE userid = '$id' AND seeder = 'no'");
	if (mysql_num_rows($res) > 0)
	  $leeching = peerstable($res);


	if ($leeching)
		print("<b></b><br />$leeching<br /><br />");
