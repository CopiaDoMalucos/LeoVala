...
$ano = date("y", utc_to_tz_time($row['date']));
	$temposeed = $POST["temposeed"];

	$temposeed = $_POST["temposeed"];
	$ret = SQL_Query_exec("INSERT INTO torrents (temposeed,filename, owner, name, filmeresolucao, filmeresolucalt, screens1, screens2, screens3, screens4, screens5, descr, filmesinopse, imdb, image1, image2, category, added, mes, dia, ano, info_hash, size, numfiles, save_as, announce, external, nfo, torrentlang, filmeano, filmeaudio, filmeextensao, filmequalidade, filme3d, filmecodecvid, filmecodecaud, filmeidiomaorigi, filmeduracaoh, filmeduracaomi, legenda, anon, tube, last_action) VALUES (".sqlesc($temposeed).",".sqlesc($fname).", '".$CURUSER['id']."', ".sqlesc($name).", ".sqlesc($filmeresolucao).", ".sqlesc($filmeresolucalt).", ".sqlesc($screens1).", ".sqlesc($screens2).", ".sqlesc($screens3).", ".sqlesc($screens4).", ".sqlesc($screens5).", ".sqlesc($descr).", ".sqlesc($filmesinopse).",'".$imdb."', '".$inames[0]."', '".$inames[1]."', '".$catid."', '" . get_date_time() . "', '" . $mes . "' , '" . $dia . "' , '" . $ano . "' , '".$infohash."', '".$torrentsize."', '".$filecount."', ".sqlesc($fname).", '".$announce."', '".$external."', '".$nfo."', '".$langid."', '".$filmeanoid."', '".$filmeaudioid."', '".$filmeextensaoid."', '".$filmequalidadeid."', '".$filme3did."','".$filmecodecvidid."', '".$filmecodecaudid."', '".$filmeidiomaorigiid."', '".$filmeduracaohid."', '".$filmeduracaomiid."', '".$legendaid."', '$anon', '".$tube."','".get_date_time()."')");
	...
	//END SCRAPE
		...
		write_loguser("Torrents-lançados","#FF0000","O torrent [url=http://www.malucos-share.org/torrents-details.php?id=".$id."]".$name."[/url] foi lançado por [url=http://www.malucos-share.org/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url]\n");
...
<form name="upload" id="upload"  enctype="multipart/form-data" action="torrents-filmes.php?cat=<?php echo $_GET["cat"] ;?>" method="post">
...
print("Portanto, antes de lançar um torrent, faça uma <a href='/http://www.malucos-share.org/torrents-pesquisa'>Pesquisa</a> para saber se ele já foi lançado.");
...
print ("</td></tr>");
print ("<tr><td width=50%  align=right  class=tab1_col3 >" . T_("TORRENT_FILE") . ": </td><td  width=50%  align=left class=tab1_col3 > <input title ='- Formato do arquivo: &lt;B&gt;.torrent&lt;/B&gt;&lt;br&gt;- Tamanho máximo: &lt;B&gt;3 MB&lt;/B&gt;&lt;br&gt;- Caso seja um relançamento, não esqueça de criar um novo .torrent' type='file' name='torrent' size='50' value='" . $_FILES['torrent']['name'] . "' />\n</td></tr>");
print ("<tr ><td width=50%  align=right  class=tab1_col3 >Torrent nome: </td><td  width=50%  align=left class=tab1_col3 ><input title ='- Título correto:&lt;br&gt;&lt;font color=red&gt;Título em Português [Título Original]&lt;/font&gt;&lt;br&gt;&lt;br&gt;- Caso não exita título traduzido, deixe apenas o título original.&lt;br&gt;&lt;br&gt;- Coloque apenas as primeiras letras em maiúsculo&lt;br&gt;&lt;br&gt;- Não adicione dados técnicos ao título.' type='text' for='name' name='name' size='40' value='" . $_POST['name'] . "' /><br />Este será retirado. torrent se estiver vazio \n</td></tr>");
...
$anoid = "<select name=\"anoteste\" id=\"anoteste\">\n<option value=\"\">Escolher</option>\n";
...
$anoid .= "</select>\n";
print("<td width='100%'  align='center'  colspan='2' class='ttable_col1'>Especificações do vídeo");
...
$filmeauid = "<select title ='- Filmes Dual Audio devem conter legenda. Caso contrário, o filme será considerado Dublado.&lt;br&gt;&lt;br&gt;- Filmes não dublados (com exceção de filmes adultos) deverão acompanhar a legenda em PT-BR.&lt;br&gt;&lt;br&gt;- Filmes dublados serão considerados duplicados caso já exista o Dual Audio.' name=\"filmeaudio\" id=\"filmeaudio\">\n<option value=\"\">Escolher</option>\n";
...
$filmeextid = "<select title ='- Se não encontrar na lista, selecione a opção OUTRO e informe a extensão do arquivo na Ficha Técnica.' name=\"filmeeste\" id=\"filmeeste\">\n<option value=\"\">Escolher</option>\n";
...
$filmequali2 = "<select title ='- O torrent não será liberado se existir um com qualidade superior já lançado, de acordo com a hierarquia:&lt;br&gt;CAM » TS » TC » Screener/R5/DVDScreen » DVDrip/BRrip&lt;br&gt;&lt;br&gt;- DVD-R, Blu-Ray e RMVB poderão ser lançados normalmente.' name=\"filmequalid\" id=\"filmequalid\">\n<option value=\"\">Escolher</option>\n";
...
$filme3did = "<select  name=\"filmes3d\" id=\"filmes3d\">\n<option value=\"\">Escolher</option>\n";
...
print ("<TR><TD width=50%  align=right  class=tab1_col3>Filme em 3D? *</td><td  width=50%  align=left class=tab1_col3>".$filme3did."</td></tr>");
/////Filme em 3D de lançamento
///// Filme legendas embutidas
$fimeslegenda = "<select name=\"legendalan\" id=\"legendalan\">\n<option value=\"\">Escolher</option>\n";
...
$filmecodecid = "<select name=\"filmecodeclan\" id=\"filmecodeclan\">\n<option value=\"\">Escolher</option>\n";
...
$filmecodevidid = "<select name=\"filmecodeaudi\" id=\"filmecodeaudi\">\n<option value=\"\">Escolher</option>\n";
...
$idiomafilme = "<select  title ='- Selecione o idioma original no qual o filme foi gravado.&lt;br&gt;&lt;br&gt;- Em caso de filmes dublados, informar o idioma original, não o da dublagem.' name=\"filmeidioma\" id=\"filmeidioma\">\n<option value=\"\">Escolher</option>\n";
...
$filmeduratio = "<select name=\"filmedurah\" id=\"filmedurah\">\n<option value=\"\">Escolher</option>\n";
...
$fimesminutosi = "<select name=\"filmedurami\" id=\"filmedurami\">\n<option  value=\"\">Escolher</option>\n";
...
///// Filme Duração minutos de lançamento
print ("<TR><TD width=50%  align=right  class=tab1_col3>Resolução: *</td><td  width=50%  align=left class=tab1_col3><input type=text name=filmeresolucao id=filmeresolucao size=4 value=" . $_POST['filmeresolucao'] . ">pixels de largura por<input type=text name=filmeresolucalt id=filmeresolucalt size=4 value=" . $_POST['filmeresolucalt'] . ">pixels de altura.<BR>\n</td></tr>");
print("<td width='100%'  align='center'  colspan='2' class='ttable_col1'>Não inclua a tag [img] nos campos abaixo.<BR>Não serão permitidas screens que não sejam do vídeo em questão.");
// screenshots //
print ("<TR><TD width=50%  align=right  class=tab1_col3>Capa: *</td><td title ='- Informe a URL da capa do filme, de preferência com boa resolução.&lt;br&gt;&lt;br&gt;Recomenda-se que as imagens sejam hospedadas no &lt;FONT COLOR=red&gt;&lt;B&gt;&lt;br&gt;tinypic.com&lt;FONT COLOR=red&gt;&lt;B&gt;&lt;br&gt;imgur.com&lt;/B&gt;&lt;/FONT&gt;' width=50%  align=left class=tab1_col3 ><input  type=text name=screens1  id=screens1  size=60 value=" . $_POST['screens1'] . " ><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 1: *</td><td  title ='- Informe a URL de uma screen real do filme. Não serão aceitas screens falsas.<br><br>- Recomenda-se que as imagens sejam hospedadas no <br><FONT COLOR=red><B>tinypic.com</B></FONT><br><FONT COLOR=red><B>imgur.com</B></FONT>' width=50%  align=left class=tab1_col3><input type=text name=screens2  id=screens2 size=60 value=" . $_POST['screens2'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 2: *</td><td  width=50%  align=left class=tab1_col3><input type=text name=screens3 id=screens3 size=60 value=" . $_POST['screens3'] . "><a href='http://imgur.com/'>imgur</a> ou <a href='https://www.flickr.com/'>flickr</a><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 3:  </td><td  width=50%  align=left class=tab1_col3><input type=text name=screens4 size=60 value=" . $_POST['screens4'] . "><BR>\n</td></tr>");
print ("<TR><TD width=50%  align=right  class=tab1_col3>Screen 4:  </td><td  width=50%  align=left class=tab1_col3><input type=text name=screens5 size=60 value=" . $_POST['screens4'] . "><BR>\n</td></tr>");
//screenshots //
...
if ($site_config['ANONYMOUSUPLOAD'] && $site_config["MEMBERSONLY"] ){ ?>
 ...
 print("<td width='100%'  align='center'  colspan='2' class='ttable_col1'>Sinopse / Ficha técnica<BR><a href='http://www.adorocinema.com/filmes/todos-filmes/notas-espectadores/'>Clique aqui</a> para obter a sinopse e ficha técnica de filmes e seriados.");

/////fim pedido de torrentes
echo "<TR title ='Forneça uma breve descrição do filme (resumo da história), facilmente encontrada em sites do gênero.' ><TD width=100%  align=center  colspan=2 class=tab1_col3  >";
echo "Sinopse: *";
...
echo "<TR title ='- Informe neste campo os dados técnicos do filme. Diretor e elenco são obrigatórios.<br><br>- Informações opcionais: Produtor, Roteiro, Estúdio, Site Oficial, etc.'><TD width=100%  align=center colspan=2  class=tab1_col3>";
echo "Ficha Técnica: *";
