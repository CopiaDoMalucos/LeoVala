...
$contact_email = "contato@malucos-share.org";
...
if (isset($_POST['envoyer']) && $_POST['envoyer'] == 'ok')
    ...
    if (!empty($nomecompleto) && !empty($nom) && !empty($mail) && !empty($indentifica) && !empty($planoescolher)  && !empty($datapag)  && $sujet != '' && $Snom == 1)
		...
		$emaildoacao="donativo@malucos-share.org";
...
EOD;
				...
				sendmail($emaildoacao, "Comprovante MS", $body, "para: $site_config[SITEEMAIL]", "-f$site_config[SITEEMAIL]");
				$mailsent = 1;
