...
<td align="center"><a href=admincp.php?action=donations><img src="images/admin/blocked.gif" border=0 width=32 height=32><BR>Barra Donativos</a><BR></td>
...
if ( $action == "messagespy" )
{
    ...
    $qry = "SELECT
                          messages.id, messages.msg, messages.added, messages.sender, messages.receiver, r.username as recipient, s.username as author
                          FROM messages
                          LEFT JOIN users r ON messages.receiver = r.id
                          LEFT JOIN users s ON messages.sender = s.id
                    WHERE messages.location IN ('in', 'both') $where ORDER BY messages.id DESC $limit";
