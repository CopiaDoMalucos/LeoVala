...
############################################################
 ...
 require_once("backend/functionsade.php"); 
 dbconn(); 
 ...
 while ($row = mysql_fetch_assoc($res)):
    ...
    $resultsub2 = mysql_query("SELECT * FROM users  LEFT JOIN usergroups ON users.id = usergroups.uid WHERE usergroups.gid = $row[id] AND usergroups.status = 'submoderadores' AND users.enabled = 'yes' AND users.status = 'confirmed'");  
	 ...
	 $resultsubqw = mysql_query("SELECT * FROM users  LEFT JOIN usergroups ON users.id = usergroups.uid WHERE usergroups.gid = $row[id] AND usergroups.status = 'administradores' AND users.enabled = 'yes' AND users.status = 'confirmed'");    
...
<table class='tab1' cellpadding='0' cellspacing='1' align='center'>
	<tbody><tr><td align="center" colspan="2" class="tab1_cab1"><?php echo $row["name"]; ?> <?php   if ($row21["status"] == 'moderadores' || $row21["status"] == 'submoderadores' || $row21["status"] == 'Administrador' || $row21["status"] == 'Sysop')
{ 
?>
...
<?php
	...
	<td width="12" class="tab1_col3" ><?php echo $online1 ;?></td>
		...
		<td width="18"  class="tab1_col3" ><a href="mailbox.php?Escrever&amp;id=<?=$row2['uid']?>"><img border="0" title="Enviar Mensagem Privada" alt="Enviar Mensagem Privada" src="images/button_pm.gif"></a></td></tr>
			<?php  
            }?>
	...
	<td width="12" class="tab1_col3" ><?php echo $online2 ;?></td>
		...
		<td width="18" class="tab1_col3" ><a href="mailbox.php?Escrever&amp;id=<?php echo $row2['uid'] ;?>"><img border="0" title="Enviar Mensagem Privada" alt="Enviar Mensagem Privada" src="images/button_pm.gif"></a></td></tr><tr>
