...
if ($edit=='1'){
    ...
    if (($type == "torrent" && $CURUSER["edit_torrents"] == "no" || $type == "news" && $CURUSER["edit_news"] == "no") && $CURUSER['id'] != $row['user'])   
		...
		print("<form method=\"post\" name=\"comment\" action=\"comentario.php?type=$type&amp;edit=1&save=1&amp;id=$id\">\n");
...
//NEWS
if ($type =="news"){
	$res = SQL_Query_exec("SELECT * FROM news WHERE newid = $id");
...
begin_framec(T_("COMMENTS"));
	...
	if ($commcount) {
		list($pagertop, $pagerbottom, $limit) = pager(10, $commcount, "comentario.php?id=$id&amp;type=$type&amp;");
		$commquery = "SELECT comments.id, text, user, comments.added, avatar, signature, username, title, class, uploaded, downloaded, privacy, donated FROM comments LEFT JOIN users ON comments.user = users.id WHERE $type = $id ORDER BY comments.added desc $limit";
	...
	if ($commcount) {
		...
		print("<br /><b>Este torrent não possui nenhum comentário.</b><br />\n");
	}

	echo "<center>";
	print("<a name='bottom'></a>");
	echo "<form name=\"comment\" method=\"post\" action=\"comentario.php?type=$type&amp;id=$id&amp;takecomment=yes\">";
