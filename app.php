...
$wherea=array();
$wherea[] = "torrents.seeders > '0'";
$where = implode(" AND ", $wherea);
///fim
    
       	$res = mysql_query("SELECT * FROM users  LEFT JOIN torrents ON users.id = torrents.owner WHERE users.class != '50' AND  torrents.safe = 'no'  AND torrents.seeders > '0' ORDER BY torrents.added ASC");     
...
print("<center>Todos os torrents pendentes foram liberados!</center>");
end_framec();  
} 
else{
begin_framec("Torrents aguardando Aprovação");
...
<div align="right">[ <a href="/app_meus.php">Meus pendentes</a> | <a href="/app_editados.php">Já editados</a> | <a href="/app_semmod.php">Sem mod responsavel</a> ]</div>
<?php
for ($i = 0; $i < $num; ++$i)
{
...
/// moderador
	  date_default_timezone_set('Etc/GMT+2');
$data12 = utc_to_tzap($tm_a['dataremodera']);
...
<td width="100" align="center" class="ttable_col2"><b>[</b><a href="#"><font size="2" ><?php echo $gerenciar ;?></font></a><b>]</B></td>
