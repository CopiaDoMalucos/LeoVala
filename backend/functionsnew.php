...
echo '<ul style="margin:0;padding:0;" class="alista">';
	...
	foreach ($cols as $col) {
		switch ($col) {
                        ...
                        break;
                        case 'name':
                                echo "<div style='text-align: left; min-width: 200px; width: 732px;' class='divhead'><a href=\"$scriptname?$addparam"."cat=".$idteam."&parent_cat=".$parent_catid."&sort=name&order=".($sort=="name" && $order=="desc"?"asc":"desc")."\"><font color='white'>Nome</font></a>".($sort=="name"?$fleche:"")."</div>";
                        break;
                        case 'dl':
	...
	foreach ($cols as $col) {
		switch ($col) {
			...
			if ($row["freeleechuser"] == 'yes'){
			 ...
			 if(($row["filmeresolucao"] >= 1152 ) && ($row["filmeresolucalt"] <= 1600))

 {
			   
                $dispname .= "<b>[<font color='#FF3635 '>HD</font>]</b>";
        }		
			 elseif(($row["filmeresolucao"] >= 1920 ) && ($row["filmeresolucalt"] <= 3200))

 {
			   
                $dispname .= "<b>[<font color='#FF3635 '>FULL HD</font>]</b>";
        }	
			 elseif(($row["filmeresolucao"] >= 3840 ) && ($row["filmeresolucalt"] <= 5120))

 {
			   
                $dispname .= "<b>[<font color='#FF3635 '>4K</font>]</b>";
        }	
					if ($row["freeleech"] == 1)
...
function pager($rpp, $count, $href, $opts = array()) {
    ...
    $mp = $pages - 1;
    $as = "<b onLoad='document.shoutboxform.message.focus()'>&lt;&lt;&nbsp;</b>";
    if ($page >= 1) {
