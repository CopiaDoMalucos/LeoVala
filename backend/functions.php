...
function userlogin() {
	...
	while ($row = mysql_fetch_assoc($res)) {
		...
		if ($banned) {
			...
			echo "<html><head><title>...::BRShares.com::...</title></head><body><h1>IP Banido</h1>Seu endereço IP está banido do sistema.<br />".
...
return sprintf( '%d meses, %d semanas, %d dia(s), %d hora(s), %d minuto(s),',  $month, $weeks, $days, $hours, $mins);
}

function seedtimenovo($ts = 0)
{    
...
function deletetorrent($id) {
	...
	SQL_Query_exec("DELETE FROM `snatched_t` WHERE `tid` = '$id'");
...
function deleteaccount($userid) 
{
	...
	SQL_Query_exec("DELETE FROM `snatched_t` WHERE `uid` = '$userid'");
...
function genrelist1() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  parent_cat='Filmes' ORDER BY name ASC ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}
...
function genrelist3() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  parent_cat='Músicas' ORDER BY name ASC ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}
...
function genrelist5() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE id='112' ");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}
...
function pesquisa_avan() {
    $ret = array();
    $res = SQL_Query_exec("SELECT id, name, parent_cat FROM categories WHERE  parent_cat='Filmes' OR  name='Seriados' ORDER BY name ASC, sort_index ASC");
    while ($row = mysql_fetch_array($res))
        $ret[] = $row;
    return $ret;
}
...
function write_logstaff($type,$couleur,$text){

	$text = sqlesc($text);
	$type = sqlesc($type);
	$couleur = sqlesc($couleur);
	$added = sqlesc(get_date_time());
	SQL_Query_exec("INSERT INTO log_staff (added, type, couleur, txt) VALUES($added, $type, $couleur, $text)");
}
...
function utc_to_tzap ($timestamp=0) {
	GLOBAL $CURUSER, $tzs;
...
$s = preg_replace("/\[mail\]((\s|.)+?)\[\/mail\]/i","<a href=\"mailto:\\1\" target=\"_blank\">\\1</a>", $s);
  ...
  // [url=http://www.example.com]Text[/url]
	...
	$s = preg_replace(
		...
		"<p class='sub'><b>citar:</b></p><table class='main' border='1' cellspacing='0' cellpadding='10'><tr><td style='border: 1px black dotted'>\\1</td></tr></table><br />", $s);

	//[quote=Author]Text[/quote]
	while (preg_match("/\[quote=(.+?)\]\s*((\s|.)+?)\s*\[\/quote\]\s*/i", $s))
	$s = preg_replace(
		"/\[quote=(.+?)\]\s*((\s|.)+?)\s*\[\/quote\]\s*/i",
		"<p class='sub'><b><div style='background-color: #f4f4f4; border-right: 1px black dotted; border-top: 1px black dotted; color: #1E5395; border-left: 4px solid #1E5395; padding: 4px;'>Citação de: \\1 </div></b></p><div style='background-color: #ffffff; border-right: 1px black dotted; border-bottom: 1px black dotted; color: #666666; border-left: 4px solid #1E5395; padding: 6px;'>\\2</div><br />", $s);
...
//fin tri
	...
	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
                        ...
                        break;
                        case 'dl':
                                echo "<td class='ttable_head' width='1' align='center'>Down</td>";
                        break;
                        case 'uploader':
                                echo "<td class=ttable_head>".T_("UPLOADER")."</td>";
                        break;
                        case 'comments':
                                echo "<td class='ttable_head' width='1' align='center'><a href=\"$scriptname?$addparam"."sort=comments&order=".($sort=="comments" && $order=="desc"?"asc":"desc")."\"><font color='white'>Com</font></a>".($sort=="comments"?$fleche:"")."</td>";
                        break;
                        case 'nfo':
                                echo "<td class=ttable_head>NFO</td>";
                        break;
                        case 'size':
                                echo "<td class='ttable_head' width='1' align='center'><a href=\"$scriptname?$addparam"."sort=size&order=".($sort=="size" && $order=="desc"?"asc":"desc")."\"><font color='white'> ".T_("SIZE")."</font></a>".($sort=="size"?$fleche:"")."</td>";
                        break;
                        case 'completed':
                        ...
                        break;
                        case 'seeders':
                                echo "<td class='ttable_head' width='1' align='center'><a href=\"$scriptname?$addparam"."sort=seeders&order=".($sort=="seeders" && $order=="desc"?"asc":"desc")."\"><font color='white'>S</font></a>".($sort=="seeders"?$fleche:"")."</td>";
                        break;
                        case 'leechers':
                                echo "<td class='ttable_head' width='1' align='center'><a href=\"$scriptname?$addparam"."sort=leechers&order=".($sort=="leechers" && $order=="desc"?"asc":"desc")."\"><font color='white'>L</font></a>".($sort=="leechers"?$fleche:"")."</td>";
                        break;
						case 'grupo':
				              echo "<th class='ttable_head'>Grupo</th>";
			            break;
	...
	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
				print("<td class='ttable_col1' align='center' valign='middle'>");
               ...
               if ($row["filmeresolucao"] > 1200 ||  $row["filmeresolucalt"] > 720 )
 {
                ...
                $dispname .= "<b>[<font color='#FF3635 '>HD</font>]</b>";
		               ...
		               if ($row["filmeresolucao"] > 1920 ||  $row["filmeresolucalt"] > 800 )
 {
			   
                $dispname .= "<b>[<font color='#FF3635 '>FULL HD</font>]</b>";
        }	
								///qualidade 3d
...
$placa="";
		...
		$postername = "<a href=account-details.php?id=".$row['user']."><b>".$row['username']."</b></a>";
		if ($postername == "") {
...
$grupos_exe="";
    ...
    $text = format_comment($row["text"]);
        ...
        if ($type == "torrent" && $CURUSER["edit_torrents"] == "yes" || $type == "news" && $CURUSER["edit_news"] == "yes" || $CURUSER['id'] == $row['user']) 
            $edit = '<a href="comments.php?id='.$row["id"].'&amp;type='.$type.'&amp;edit=1"><img src="themes/'.$THEME.'/forums/p_edit.png"  alt="Editar"></a>&nbsp;';
       
        $delete = null;
        if ($type == "torrent" && $CURUSER["delete_torrents"] == "yes" || $type == "news" && $CURUSER["class"] == "95")
            $delete = '<a href="comments.php?id='.$row["id"].'&amp;type='.$type.'&amp;delete=1"><img src="themes/'.$THEME.'/forums/p_delete.png"  alt="Deletar"></a>&nbsp;';
   ...
   print('<td class="f-border comment-details" align="left" width="130"><center><img  src="'.$avatar.'" alt="" /><br /><font class=tipo_forum>'.$grupos_exe.'</font><center><br><font class=tipo_forum><b>'.$placa.'</B></font><center><font class=tipo_forum></font></center><br><font class=tipo_forum>Ratio: </font> <B><font class=tipo_forum> '.$userratio.' </font></B><center></font><font color=#151515>[</font> <font color=#00CC00><B> '.$useruploaded.' </B></font><font color=#151515>]</font><font color=#151515>|</font><font color=#151515>[</font><font color=#FF0000><B> '.$userdownloaded.' </B></font><font color=#151515><font color=#151515>]</font></center><center><font class=tipo_forum></font></center></center><center><br><font class=tipo_forum>Lançamentos:  </font><B><font class=tipo_forum> ' . number_format($numtorrents) . ' </font></B></center><center><font class=tipo_forum>Comentários:  <B> ' . number_format($numcomments) . ' </font></B><br></center><br><center><B> '.$online.' </B><br></center></td>');
        ...
        print('<td align="center" width="17%" class="comenttorre"><a href="account-details.php?id='.$row["user"].'"><img src="themes/'.$THEME.'/forums/icon_profile.gif" border="" alt="" /></a> <a href="enviarmp.php?receiver='.$row["username"].'"><img src="themes/'.$THEME.'/forums/icon_pm.gif" border="0" alt="" /></a></center></td>');
