...
$res1 = mysql_query("SELECT torrents.id, torrents.size, torrents.safe, torrents.screens1, torrents.name, torrents.owner, torrents.userup FROM torrents WHERE id IN ($ids)");
...
$torrentid = $arr1["id"];
$torrentscreem = $arr1["screens1"];
			write_loguser("Auto-liberado","#FF0000","O torrent [url=http://www.brshares.com/torrents-details.php?id=".$torrentid."]".$torrentname."[/url] foi liberado por [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url]\n");
               ...
               if ($row_qualidade["filmeresolucao"] > 1200 ||  $row_qualidade["filmeresolucalt"] > 720 )
 {
		...
		$message = "[color=green][url=http://www.brshares.com/torrents-details.php?id=".$torrentid."]".$torrentname."[/url][/color]  foi enviado por [url=http://www.brshares.com/account-details.php?id=".$arr1["owner"]."]".$arr1["userup"]."[/url]";
				 $userupload="INSERT INTO `shoutbox` (`userid`, `user`, `message`, `date`) VALUES ('0', 'Anúncio!', " . sqlesc($message) . ", '".get_date_time()."')";
                      @mysql_query($userupload);
