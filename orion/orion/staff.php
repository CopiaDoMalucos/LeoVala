...
############################################################
  ...
  $res = SQL_Query_exec("SELECT `users`.`id`, `users`.`username`, `users`.`class`, `users`.`last_access` FROM `users` INNER JOIN `groups` ON `users`.`class` = `groups`.`group_id`  WHERE `users`.`enabled` = 'yes' AND `users`.`status` = 'confirmed' AND `users`.`username` != 'admin' AND `groups`.`staff_page` = 'yes' ORDER BY `username`");
  ...
  stdhead(T_("STAFF"));
  begin_framec(T_("STAFF"));
  ?>
<font size="2" ><center>Essa é a nossa Equipe BRShares, estamos felizes por sua visita e à disposição para quaisquer dúvidas, basta mandar uma MP!!!<br></center></font>
