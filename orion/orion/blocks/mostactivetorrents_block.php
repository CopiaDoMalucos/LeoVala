...
print("<table width=100% border=0 valign=top></CENTER><DIV><tr><CENTER>");
	if ($latestuploadsrecords) {
		foreach ($latestuploadsrecords as $row) { 
			...
			$img1 = "</CENTER><BR><BR><a href='{$site_config[SITEURL]}/torrents-details.php?id=$row[id]'><img class='imgdestaques' border='0'src='{$row[screens1]}' title=\"".$ler."\" alt=\"$altname / $cat\" height=150 width=120></a><br></b><br><a href='torrents-details.php?id=$row[id]' title='".htmlspecialchars($row["name"])."'>$smallname</a><br />\n";
	...
	print("<TR><td align=center>" .$img1. "</td>");
		...
		print("<center>Nada encontrado</center>\n");
	}
