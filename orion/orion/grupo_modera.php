...
############################################################
 ...
 if (mysql_num_rows($res) == 0)
     show_error_msg("Erro", "Esse grupo não existe..", 1);
...
if ($row21["gid"] !=  $id ){
     show_error_msg("Erro", "Você não tem acesso a este grupo", 1);
}
        ...
        if ($status == 'retirar'){
			 ...
			 if ($rowverificar["status"] == 'submoderadores'){
              ...
              if ($gruclassub < $gruclasmod){
			  	   print("<b><center>Desculpe, mais você não pode rebaixar o moderador do grupo!!!<a href='grupo_modera.php?id=$id'><br>Voltar</a></center></b>");
					end_framec();
	                stdfoot();
	                die();
			  }	
			 ...
			 if ($userid == $CURUSER["id"]){
			 		print("<b><center>Desculpe, mais você não pode mudar seu status dentro do grupo!!!<a href='grupo_modera.php?id=$id'><br>Voltar</a></center></b>");
					end_framec();
	                stdfoot();
	                die();
			 }
		     ...
		     mysql_query("INSERT INTO grupodel (gruid, gruuserid, grudata) VALUES($useidteam, $userid, $dt)");
							...
							mysql_query("UPDATE users SET  team='0' WHERE id=$userid ");
							    ...
							    $message = 'Prezado, /n/n Você acaba de ser retirado do grupo - ' . htmlspecialchars($row['name']);
                    			$dt = sqlesc(get_date_time()); 
		...
		if ($status == 'membrogrupo'){
			 ...
			 if ($userid == $CURUSER["id"]){
	                ...
	                die();
								...
								if (mysql_num_rows($res1234) > 0){
			                    mysql_query("UPDATE `usergroups` SET `status` = 'membrogrupo',`chave` = '1' WHERE `gid` = $useidteam AND `uid` = $userid");  
                                $message = 'Parabéns,/n/n Você acaba de ser promovido a membro do grupo - ' . htmlspecialchars($row['name']);
                    			$dt = sqlesc(get_date_time()); 
                                $sub = sqlesc($row['name'] . ' Grupo');
                                $msg = sqlesc($message);
  			                    mysql_query("INSERT INTO messages (sender, receiver, added, msg, poster, subject) VALUES(0, $userid, $dt, $msg, 0, $sub)");  									
			...
			$verificar = mysql_query("SELECT * FROM `usergroups` WHERE `uid` = " . $CURUSER["id"] . "");
			 ...
			 if ($userid == $CURUSER["id"]){
	                ...
	                die();
                                ...
                                $message = 'Parabéns, /n/n Você acaba de ser promovido a submoderador de grupo do  - ' . htmlspecialchars($row['name']);
                    			$dt = sqlesc(get_date_time()); 
                                $sub = sqlesc($row['name'] . ' Grupo');
                                $msg = sqlesc($message);
  			                    mysql_query("INSERT INTO messages (sender, receiver, added, msg, poster, subject) VALUES(0, $userid, $dt, $msg, 0, $sub)");  
			...
			$verificar = mysql_query("SELECT * FROM `usergroups` WHERE `uid` = " . $CURUSER["id"] . "");
			 ...
			 if ($userid == $CURUSER["id"]){
	                ...
	                die();
                                ...
                                $message = 'Parabéns, /n/n Você acaba de ser promovido a moderador de grupo do  - ' . htmlspecialchars($row['name']);
                    			$dt = sqlesc(get_date_time()); 
                                $sub = sqlesc($row['name'] . ' Grupo');
                                $msg = sqlesc($message);
  			                    mysql_query("INSERT INTO messages (sender, receiver, added, msg, poster, subject) VALUES(0, $userid, $dt, $msg, 0, $sub)");  
...
echo("<td class='ttable_col2' width='10%' align='center'>$uploaded<input type='hidden' name='ditarmembro' value='true' /></td>");
 ...
 print("<a href=painel_grupos.php?id=$id><font color=#FF0000><CENTER><b>[Voltar painel de grupo]</b></CENTER></font></a>");
           ...
           echo('<center>Não temos membros no grupo</center>'); 
        }
	...
	end_framec();
      ...
      show_error_msg("Erro", "Você não tem permissão para isso.", 1);
}
	stdfoot();
