...
$select_row->execute(); 
   $row_select = $select_row->fetch(PDO::FETCH_ASSOC);  
			...
			if ($doapontos == '50' || $doapontos == '100' || $doapontos == '200' || $doapontos == '300' || $doapontos == '500' || $doapontos == '600' || $doapontos == '1000'){
			  ...
			  write_logstaff("Bonus","#FF0000","O usuário [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url] enviou ".$doapontos." pontos para o [url=http://www.brshares.com/account-details.php?id=".$userid."]".$username."[/url] \n");

			  $msg = "Prezado usuário,\n\nO usuário [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url] fez uma doação de ".$doapontos." BR Pontos para você!\n\nOs pontos já foram adicionados em sua conta.\n\n Atenciosamente,\n\n Equipe BR ";

			  
			      $added = get_date_time();
    $subject  = "Promovido";
	
	$mpbonus=$pdo->prepare("INSERT INTO messages (poster, sender, receiver, msg, added,subject) VALUES ('0','0', " . $userid . ", " .sqlesc($msg) . ", '" . get_date_time() . "','BR Pontos!')");			  
    $mpbonus->execute();   
...
stdhead(T_("SEED_BONUS_MS"));
  ...
  print("<CENTER>(".T_("MS_BONUS_TEM")."<b><font color=red> " . number_format($CURUSER["seedbonus"], 2) . "</font></b>) ".T_("MS_BONUS_PONTOS")."</CENTER><br />");
  echo("<table>");
  echo("<tr>");
  echo("<td ><b><font size='2'>Como funciona?</font></b><br><br> 

A cada <b>1 hora</b> que você estiver semeando arquivos no BR, você ganhará <b>1 BR Ponto</b>. Você também pode ganhar BR Pontos fazendo lançamentos de torrents: 2 BR Pontos (arquivos até 10mb); 4 BR Pontos (arquivos até 100mb); 5 BR Pontos (arquivos acima de 100mb).
<br><br>
Acumulando BR Pontos, você poderá trocá-los por <b>GB de upload</b> ou <b>convites</b>!<br><br><b><font size='2'>Observações</font></b><br><br> 
...
$sql = $pdo->query("SELECT * FROM bonus ORDER BY id");
  ...
  echo("<br>");
    echo("<table id='tabela1' cellpadding='0' cellspacing='1'  width='100%' align='center'>");
  echo("<td class='tab1_cab1' align='center' colspan='3'>Doar BR Pontos</td>");
...
</select> BR Pontos para o usuário <input type='text' name='username' size='30'/></td>");
echo("</tr>");
echo("<tr>");
