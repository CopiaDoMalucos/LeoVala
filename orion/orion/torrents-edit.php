...
if($CURUSER["level"]=="Administrador" || $CURUSER["level"]=="Sysop" || $CURUSER["level"]=="Moderador" || $CURUSER["level"]=="S.Moderador" || $CURUSER["level"]=="Liberador"){
	...
	$msg = "Torrent deletado!\n\n O torrent [size=2][color=green]$torrentname [/color][/size]  foi deletado!!! \n\n Razão: $delreason.\n";
...
mysql_query($sql);
	...
	write_loguser("Torrents-deletados","#FF0000","O torrent [url=http://www.brshares.com/torrents-details.php?id=".$id."]".$row["name"]."[/url] foi deletado por [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url] Razão: $delreason\n");
...
SQL_Query_exec("INSERT INTO messages (poster, sender, receiver, msg, added,subject) VALUES('0','0', " . $testeapro['uid'] . ", " .sqlesc($msg) . ", '" . get_date_time() . "','Torrent moderação!')") or die (mysql_error());
	...
	write_loguser("Torrents-editados","#FF0000","O torrent [url=http://www.brshares.com/torrents-details.php?id=".$id."]".$row["name"]."[/url] foi editado por Por [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url]\n");
...
if($CURUSER["level"]=="Administrador" || $CURUSER["level"]=="Moderador" || $CURUSER["level"]=="Sysop" ){
    echo "<tr><td width=50%  align=right  class=tab1_col3><B>Recomendar: </b></TD><TD width=50%  align=left class=tab1_col3 ><input type=\"checkbox\" name=\"recommended\"" . (($row["recommended"] == "yes") ? " checked=\"checked\"" : "" ) . " value=\"1\" />Destaques BRShares</TD></TR>";
}
// END RECOMMENDED TORRENTS ///
	}
