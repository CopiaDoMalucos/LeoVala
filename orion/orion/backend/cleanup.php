...
$timenow = get_date_time();
		...
		Você também pode fazer uma doação para o site, onde tem diversas opções, onde pode adquirir GB de Upload para ser somado à sua conta, aumentando assim seu ratio. Clique no link abaixo e escolha a melhor opção para você.

		[url=http://www.brshares.com/donate.php]Faça uma doação[/url]
...
/*
	...
	*/
    ...
    if (mysql_num_rows($query) > 0)
    {
        ...
        while ($row = mysql_fetch_array($query))
        {
            mysql_query("UPDATE `users` SET `freeleechuser` = 'no', `freeleechexpire` = '0000-00-00 00:00:00', donated='0.00', donator='n' WHERE `id` = " . $row["id"] . "") or die (mysql_error());
   ...
   $delretira = "DELETE FROM donate WHERE uid =  " . $row["id"] . " ";
...
$res_deleteuser = SQL_Query_exec("SELECT * FROM users WHERE enabled = 'yes' AND last_access < FROM_UNIXTIME($ide_deketeuser) AND added < FROM_UNIXTIME($ide_dever)  LIMIT 10 ");
...
Informar que sua conta do site www.brshares.com foi cancelada devido a inatividade(mais de 90 dias sem acesso), caso deseje participar novamente da comunidade, poderá efetuar um novo cadastro, esse deverá ser através de convite de um membro existente. Pode ser solicitado também através da nossa comunidade oficial no Facebook http://www.facebook.com/groups/242957942441666/

Caso tenha dúvidas entre em contato conosco, será um prazer atende-lo.
contato@brshares.com
...
EOD;
				...
				sendmail($emaildel, "Sua conta foi desativada BR-Shares", $body, "para: $site_config[SITEEMAIL]", "-f$site_config[SITEEMAIL]");
				$mailsent = 1;
