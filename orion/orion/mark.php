...
$res1 = mysql_query("SELECT torrents.id, torrents.size, torrents.safe, torrents.screens1, torrents.name, torrents.owner, torrents.userup  FROM torrents WHERE id='".$id."' AND safe='no'");
...
mysql_query("UPDATE torrents SET safe='yes', markdate='".get_date_time()."' WHERE id=$id") or die(mysql_error());
 ...
 if($arr1["size"] >= 4294967296){
					...
					$pontos = (int) 0;
						...
						write_loguser("Torrents-liberados","#FF0000","O torrent [url=http://www.brshares.com/torrents-details.php?id=".$id."]".$torrentname."[/url] foi liberado por [url=http://www.brshares.com/account-details.php?id=".$CURUSER["id"]."]".$CURUSER["username"]."[/url] \n");
				...
				$message = "[color=green][url=http://www.brshares.com/torrents-details.php?id=".$id."]".$torrentname."[/url][/color]  foi enviado por [url=http://www.brshares.com/account-details.php?id=".$arr1["owner"]."]".$arr1["userup"]."[/url]";
