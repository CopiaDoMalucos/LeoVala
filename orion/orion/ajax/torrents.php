...
require_once("backend/functionsnew.php");
  ...
  switch ( $_GET[ 'tid' ] )
  {
	  ...
	  case 3:  $where[] = "torrents.category IN (9,111,113)";
           break;   
		   
	  case 4:  $where[] = "torrents.category IN (47,104,106)";
           break;   
  }
...
$query3 = mysql_query("SHOW COLUMNS FROM users WHERE Field = 'vertorr'");
$row3 = mysql_fetch_array($query3);
$enum = str_replace("enum(", "", $row3['Type']);
$enum = str_replace("'", "", $enum);
$enum = substr($enum, 0, strlen($enum) - 1);
$enum = explode(",", $enum);
echo "<div align='right'><select onchange='location = options[selectedIndex].value'  name='vertorr" . $CURUSER['id'] . "'>";
foreach ($enum as $chave => $campo) {
if($campo == $CURUSER['vertorr'] ){
  echo '<option selected="selected" value="?tor='.$campo.'">Ver '.$campo.' torrents</option>';
 }else{
  echo '<option value="?tor='.$campo.'">Ver '.$campo.' torrents</option>';
 }
}
echo '</select></div>';
       return;
