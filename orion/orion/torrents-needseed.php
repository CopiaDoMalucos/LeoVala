...
############################################################
 ...
 $res = SQL_Query_exec("SELECT torrents.id, torrents.name, torrents.owner, torrents.external, torrents.size, torrents.seeders, torrents.leechers, torrents.safe, torrents.times_completed, torrents.added, users.username FROM torrents LEFT JOIN users ON torrents.owner = users.id WHERE torrents.banned = 'no' AND safe='yes' AND torrents.leechers > 0 AND torrents.seeders <= 0 ORDER BY torrents.seeders");
 
 if (mysql_num_rows($res) == 0)
     show_error_msg(T_("ERROR"), T_("NO_TORRENT_NEED_SEED"), 1);
     
     stdhead(T_("TORRENT_NEED_SEED"));
     begin_framec(T_("TORRENT_NEED_SEED"));
